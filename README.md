...
**The picker is the day's board.** It reads the same array the To Do, In Progress, Blocked and Finished columns are drawing, with the board's own columns as its filters — so the two can never describe a day differently. It used to read every task in the history, which is how a copy of the same work from twelve days ago sat next to today's; carry-forward already brings anything unfinished onto today, so the board is the complete picture and one record per task falls out for free.

**The canvas is the day's board.** Open it and every task on that day is already there as a node — nothing to pull in one at a time. Taking one off (✕) is remembered for that day, so the sync never puts it back; **Show all** undoes every one of those, **Clear** takes the day off the canvas. Being on the canvas is a view, never an edit: no task changes status just by appearing, because a page that quietly moves eight things into In Progress the moment you open it is a page nobody can trust. Status changes come from the board, or from a node's own buttons.
...
**Draw the arrows yourself**, two ways: drag the connector on a card's right edge onto another card, or press **→ Link** and then click the task that comes after. The second exists because the first is a fourteen-pixel target and a drag, which is fine with a mouse and miserable on a trackpad. Arrows come off the same way they go on: hover one and a **✕** appears at its midpoint — clicking the line still works, but a two-pixel line is not an affordance anyone can find. **Any node links to any node** — the board's own resolver already follows a dependency across days and lineage, and each carry rewrites the id onto that day's copy, so the graph refusing cross-day links was the view being stricter than the thing it views. The only refusal left is a loop. It writes the board's own `dependsOn` — the same field the task panel edits and the board warns on — so there is no second notion of *comes before* anywhere in the app. Loops are refused, and two cards from different days can't be joined, because dependencies here have always been day-local.
